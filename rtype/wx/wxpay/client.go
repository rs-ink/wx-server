...
func (c *Client) fillRequestData(params Params) Params {
	if _, ok := params["wxappid"]; !ok {
		params["appid"] = c.account.appID
		if _, ok = params["bill_type"]; !ok {
...
func (c *Client) SendRedPack(params Params) (Params, error) {
	...
	if err != nil {
		...
		log.Warn(params)
		log.Warn(utf8.RuneCountInString(params["act_name"]))
		log.Warn(xmlStr)
...
//发裂变红包
func (c *Client) SendGroupPack(params Params) (Params, error) {
	var url string
	if c.account.isSandbox {
		url = SandBoxSendGroupRedPack
	} else {
...
func (c *Client) GetRedPackInfo(params Params) (Params, error) {
	var url string
	if c.account.isSandbox {
		url = SandBoxGetHbInfo
	} else {
		url = GetHbInfo
	}
	xmlStr, err := c.postWithCert(url, params)
	if err != nil {
		...
		log.Warn(xmlStr)
		var res WxResultSendRedPack
		err := xml.Unmarshal([]byte(xmlStr), &res)
		log.CheckShowError(err)
		log.Warn(res)
		return XmlToMap(xmlStr), nil
	}
}
